...
np.random.seed(123)
design_rep = 100
infer_results = np.empty((2, design_rep*5, 8))
k0_true = 2
k_input_set = 5
...
for ii in range(design_rep):
    ...
    for k_eg in range(k_input_set):
        ...
        def process_single_iteration():
            ...
            Y = gen_outcome_mismatch(k0=k0_true,
        ...
        infer_results[0, design_rep*k_eg + ii] = infer_smry_add(result=results_array_1,
                                                                tau_true_value=tau_true)
        infer_results[1, design_rep * k_eg + ii] = infer_smry_add(result=results_array_2,
                                                                  tau_true_value=tau_true)
